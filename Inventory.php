...
class Inventory {
	...
	public function listPurchase() {
		...
		while ($purchase = mysqli_fetch_assoc($result)) {
			$purchaseRow = array();
			$purchaseRow[] = $purchase['purchase_id'];
			$purchaseRow[] = $purchase['pname'];
			$purchaseRow[] = $purchase['quantity'];
			$purchaseRow[] = $purchase['supplier_name'];
			$purchaseRow[] = '<div class="btn-group btn-group-sm"><button type="button" name="update" id="' . $purchase["purchase_id"] . '" class="btn btn-primary btn-sm rounded-0 update" title="Update"><i class="fa fa-edit"></i></button><button type="button" name="delete" id="' . $purchase["purchase_id"] . '" class="btn btn-danger btn-sm rounded-0 delete" title="Delete"><i class="fa fa-trash"></i></button></div>';
...
{		
    ...
    if(!empty($_POST["search"]["value"])) {
        $searchValue = $_POST["search"]["value"];
        $sqlQuery .= "WHERE o.order_id LIKE '%".$searchValue."%' ";
        $sqlQuery .= "OR p.pname LIKE '%".$searchValue."%' ";
        $sqlQuery .= "OR o.total_shipped LIKE '%".$searchValue."%' ";
...
{		
    $sqlQuery = "SELECT p.pid, p.pname, p.model, p.quantity as product_quantity, COALESCE(s.quantity, 0) as received_quantity, COALESCE(r.total_shipped, 0) as total_shipped
    ...
    if (!empty($searchValue)) {
        $sqlQuery .= "WHERE p.pname LIKE '%".$searchValue."%' OR p.model LIKE '%".$searchValue."%'";
    }
...
public function updateAccount() {
    if(isset($_POST['userid'])) {
        ...
        $query = "UPDATE registration SET Fullname = '$fullname', Position = '$position', Email = '$email', Address = '$address', Phone = '$phone', Company = '$company', Username = '$username', Password = '$password' WHERE ID = '".$_POST['userid']."'";
